...
public class TaskDaoTests {
	...
	public void testCreateRetrieve() {
		...
		Task t1 = new Task("My first task", "This is a task", rightnow, stamp,
				category, priority, state, user, user, "okey", 0);
		Task t2 = new Task("My second task", "This is a task", date, stamp,
				category, priority, state, user, user, "okey", 0);
		Task t3 = new Task("My third task", "This is a task", datesql, stamp,
				category, priority, state, user, user, "okey", 0);
		...
		System.out.println("Date before/ >"
				+ ((t1.getDeadline().getTime() / 1000) * 1000)
				+ " - Before TS/>"
				+ ((t1.getDeadline().getTime() / 1000) * 1000));
		System.out.println("Date after / >"
				+ ((taskR1.getDate().getTime() / 1000) * 1000)
				+ " - After TS/ >"
				+ ((taskR1.getDeadline().getTime() / 1000) * 1000));
		...
		assertEquals("Retrieved task should be same hashCode inserted task.",
				taskR1.hashCode(), t1.hashCode());
	}
	...
	public void testUpdate() {
		...
		Task t1 = new Task("My first task", "This is a task", rightnow, stamp,
				category, priority, state, user, user, "okey", 0);
		Task t2 = new Task("My second task", "This is a task", rightnow, stamp,
				category, priority, state, user, user, "okey", 0);
	...
	/*
	 * @Test public void testTimestampVsDate() { java.util.Date date = new
	 * java.util.Date(); java.util.Date stamp = new
	 * java.sql.Timestamp(date.getTime()); assertTrue("date.equals(stamp)",
	 * date.equals(stamp)); assertTrue("stamp.compareTo(date)",
	 * stamp.compareTo(date) == 0); assertTrue("date.compareTo(stamp)",
	 * date.compareTo(stamp) == 0); assertTrue("stamp.equals(date)",
	 * stamp.equals(date)); }
	 * 
	 * @Test public void testTimestampVsDate1() { java.util.Date date = new
	 * java.util.Date(); java.util.Date stamp = new
	 * java.sql.Timestamp(date.getTime()); assertTrue("date.equals(stamp)",
	 * date.equals(stamp)); }
	 * 
	 * @Test public void testTimestampVsDate2() { java.util.Date date = new
	 * java.util.Date(); java.util.Date stamp = new
	 * java.sql.Timestamp(date.getTime()); assertTrue("stamp.compareTo(date)",
	 * stamp.compareTo(date) == 0); }
	 * 
	 * @Test public void testTimestampVsDate3() { java.util.Date date = new
	 * java.util.Date(); java.util.Date stamp = new
	 * java.sql.Timestamp(date.getTime()); assertTrue("date.compareTo(stamp)",
	 * date.compareTo(stamp) == 0); }
	 * 
	 * @Test public void testTimestampVsDate4() { java.util.Date date = new
	 * java.util.Date(); java.util.Date stamp = new
	 * java.sql.Timestamp(date.getTime()); assertTrue("stamp.equals(date)",
	 * stamp.equals(date)); }
	 */
}
